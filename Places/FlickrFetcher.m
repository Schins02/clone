...
+ (NSURL *)URLforRandomPlace
{
    //roughly the US
    int minLatitude = 30;
    int maxLatitude = 50;
    int minLongitude = -120;
    int maxLongitude = -80;
    
    long double latitude = (abs(arc4random()) % (maxLatitude - minLatitude)) + minLatitude + drand48();
    long double longitude = (abs(arc4random()) % (maxLongitude - minLongitude)) + minLongitude + drand48();
    
    NSString* query = @"https://api.flickr.com/services/rest/?method=flickr.places.findByLatLon&";
    query = [query stringByAppendingString:[NSString stringWithFormat:@"lat=%Lf&lon=%Lf",latitude,longitude]];
